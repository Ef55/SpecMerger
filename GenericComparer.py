...
from utils import GenericParsedPage
from typing import Optional, Callable
import nltk


def group_by(l: list | set, key_function):
    result = {}
    for elem in l:
...
class Aligner:
        ...
        for i in range(max(len(left.entries_list), len(right.entries_list))):
            ...
            if key_left == key_right:
                ...
                if key_left is not None and key_left in right.entries.keys() and key_left not in ordered_keys:
                    ...
                    left_entry = left.entries[key_left]
    ...
    def align_dict(self, left: Content, right: Content) -> Content:
        ...
        left: Dictionnary
        right: Dictionnary
        keys_left = set(left.entries.keys())
        keys_right = set(right.entries.keys())
        common_keys = keys_left & keys_right
        for key in common_keys:
            if key == "22.2.1.7":
                print("DEBUGGG")
            current_dic[key] = self.align(left.entries[key], right.entries[key])
        remaining_left = keys_left - common_keys
        remaining_right = keys_right - common_keys
        remaining_keys = remaining_left | remaining_right
        used = set()
        for key in remaining_keys:
            ...
            if key in remaining_left:
                match Aligner.find_closest_key(remaining_right - (used | remaining_left), key):
                    case None:
                ...
                match Aligner.find_closest_key(remaining_left - (used | remaining_right), key):
                    case None:
    ...
    def align_set(self, left: Content, right: Content) -> Content:
        left: Bag
        right: Bag
        constructing_set: set[Content] = set()
        elems_left = left.bag
        ...
        for elem_left in elems_left:
            for elem_right in elems_right:
                if elem_left == elem_right:
                    ...
                    break
        constructing_set.update(common_elems)
        missing_left = elems_right - common_elems
        missing_left_grouped_by_type = group_by(missing_left, type)
        missing_right = elems_left - common_elems
        missing_right_grouped_by_type = group_by(missing_right, type)
        for elem in missing_left_grouped_by_type.keys() | missing_right_grouped_by_type.keys():
            left_elems = missing_left_grouped_by_type.get(elem, [])
            right_elems = missing_right_grouped_by_type.get(elem, [])
            if len(left_elems) == 0:
                constructing_set.update(
                    AlignmentNotPossible((None,None),None, elem, GenericReportErrorType.MISSING_IN_BAG) for elem in right_elems)
            elif len(right_elems) == 0:
                constructing_set.update(
                    AlignmentNotPossible((None,None),elem, None, GenericReportErrorType.MISSING_IN_BAG) for elem in left_elems)
            elif len(left_elems) == len(right_elems) == 1:
                subtree = self.align(left_elems[0], right_elems[0])
                match subtree:
                    case AlignmentNotPossible(_, _, _):
                        constructing_set.add(AlignmentNotPossible((None,None),left_elems[0], None, GenericReportErrorType.MISSING_IN_BAG))
                        constructing_set.add(
                            AlignmentNotPossible((None,None),None, right_elems[0], GenericReportErrorType.MISSING_IN_BAG))
                    case _:
                        constructing_set.add(AlignmentError(subtree, GenericReportErrorType.MISSPELLED_ENTRY))
            else:
                constructing_set.update(AlignmentNotPossible((None,None),left_elem, None, GenericReportErrorType.MISSING_IN_BAG)
                                        for left_elem in left_elems)
                constructing_set.update(AlignmentNotPossible((None,None),None, right_elem, GenericReportErrorType.MISSING_IN_BAG)
